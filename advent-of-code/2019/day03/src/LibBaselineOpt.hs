...
module LibBaselineOpt (run, doPart1, doBench) where
...
-- wireCoords :: [Move] -> [Coord]
-- -- wireCoords =
-- -- go 0 0 0 0 0
-- -- where
-- --   go x y _ _ 0 [] = [Coord x y]
-- --   go x y _ _ 0 (MRight v : tl) = go x y 1 0 v tl
-- --   go x y _ _ 0 (MLeft v : tl) = go x y (-1) 0 v tl
-- --   go x y _ _ 0 (MUp v : tl) = go x y 0 1 v tl
-- --   go x y _ _ 0 (MDown v : tl) = go x y 0 (-1) v tl
-- --   go x y dx dy t tl = Coord x y : go (x + dx) (y + dy) dx dy (t - 1) tl
-- -- go 0 0
-- -- where
-- --   go x y [] = [Coord x y]
...
-- --   go x y (MRight v : tl) = Coord x y : go (x + 1) y (MRight (v - 1) : tl)
-- --   go x y (MLeft v : tl) = Coord x y : go (x - 1) y (MLeft (v - 1) : tl)
-- --   go x y (MUp v : tl) = Coord x y : go x (y + 1) (MUp (v - 1) : tl)
-- --   go x y (MDown v : tl) = Coord x y : go x (y - 1) (MDown (v - 1) : tl)
...
wireCoords =
  ...
  where
    go !x !y [] = [Coord x y]
    go !x !y (MRight !v : tl) = go2 x y 1 0 v tl
    go !x !y (MDown !v : tl) = go2 x y 0 (-1) v tl
    go !x !y (MLeft !v : tl) = go2 x y (-1) 0 v tl
    go !x !y (MUp !v : tl) = go2 x y 0 1 v tl
    go2 !x !y _ _ 0 tl = go x y tl
    go2 !x !y !dx !dy !t tl = Coord x y : go2 (x + dx) (y + dy) dx dy (t - 1) tl
...
doPart1 input = do
  let (moves1, moves2) = parseInput input
      ...
      (Coord closestX closestY) = L.minimumBy (comparing manhattan) $ inters
      closestManhattan = manhattan (Coord closestX closestY)
